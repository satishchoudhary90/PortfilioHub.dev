  ...
  return (
    <div className="min-h-screen bg-white text-black">
      <section className="min-h-[90vh] flex items-center px-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className="max-w-6xl mx-auto w-full">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              {user.image && (
                <img src={user.image} alt={user.name} className="w-full max-w-md aspect-square object-cover grayscale" />
              )}
            </div>
            <div>
              <h1 className="text-6xl md:text-8xl font-black leading-[0.9] tracking-tighter uppercase">{user.name}</h1>
              {user.headline && <p className="text-xl text-gray-500 mt-6 font-light">{user.headline}</p>}
            ...
            <div className="flex flex-wrap gap-4">
              {user.skills.map((skill: any) => (
                <FadeInView key={skill.id}>
                  <span className="text-2xl font-light text-gray-800 hover:text-black transition-colors">{skill.name}</span>
                </FadeInView>
              ))}
            </div>
          </div>
                ...
                <FadeInView key={project.id}>
                  <div className="grid md:grid-cols-2 gap-8 items-start">
                    <div className={index % 2 === 1 ? "md:order-2" : ""}>
                      {project.imageUrl ? (
                        <img src={project.imageUrl} alt={project.title} className="w-full aspect-video object-cover grayscale hover:grayscale-0 transition-all duration-700" />
                      ) : (
                        <div className="w-full aspect-video bg-gray-100 flex items-center justify-center text-gray-300 text-6xl font-black">{(index + 1).toString().padStart(2, "0")}</div>
                      )}
                    </div>
                    <div className={index % 2 === 1 ? "md:order-1" : ""}>
                      <span className="text-sm text-gray-400">{(index + 1).toString().padStart(2, "0")}</span>
                      <h3 className="text-3xl font-black uppercase tracking-tight mt-2">{project.title}</h3>
              ...
              {user.educations.map((edu: any) => (
                <FadeInView key={edu.id}>
                  <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2">
                      <h3 className="text-xl font-bold">{edu.degree}{edu.field ? ` in ${edu.field}` : ""}</h3>
          ...
          <FadeInView>
            ...
            {sent ? (
              ...
              <form onSubmit={handleContact} className="space-y-6">
                <input value={contactForm.name} onChange={(e) => setContactForm(p => ({ ...p, name: e.target.value }))} placeholder="Name" required className="w-full p-4 bg-transparent border-b border-gray-800 text-white placeholder-gray-600 focus:border-white outline-none" />
                <input value={contactForm.email} onChange={(e) => setContactForm(p => ({ ...p, email: e.target.value }))} placeholder="Email" type="email" required className="w-full p-4 bg-transparent border-b border-gray-800 text-white placeholder-gray-600 focus:border-white outline-none" />
                <textarea value={contactForm.message} onChange={(e) => setContactForm(p => ({ ...p, message: e.target.value }))} placeholder="Message" rows={4} required className="w-full p-4 bg-transparent border-b border-gray-800 text-white placeholder-gray-600 focus:border-white outline-none resize-none" />
                <button type="submit" disabled={sending} className="w-full py-4 bg-white text-black font-bold uppercase tracking-widest hover:bg-gray-200 transition-colors">{sending ? "Sending..." : "Send"}</button>
              </form>
            )}
          </FadeInView>
        </div>
