...
export default function OceanBlueTemplate({ data }: { data: { user: any } }) {
  const { user } = data;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-950 via-slate-900 to-blue-950 text-white">
        ...
        <section className="py-24 px-6">
          <FadeInView>
            <div className="max-w-4xl mx-auto">
              ...
              <p className="text-lg text-blue-100/90 leading-relaxed">{user.bio}</p>
            </div>
          </FadeInView>
        </section>
      )}
              ...
              {user.projects.map((project: any) => (
                <FadeInView key={project.id}>
                  <div className="group rounded-2xl bg-white/5 border border-blue-500/20 p-6 hover:border-blue-400/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.15)] transition-all">
                    ...
                    {project.description && (
                      <p className="text-blue-200/70 mt-2 text-sm line-clamp-3">{project.description}</p>
                    )}
                    {project.techStack?.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4">
                        {project.techStack.map((tech: string, i: number) => (
                          <span
                            key={i}
                            className="px-2 py-1 text-xs rounded-md bg-blue-500/10 text-cyan-300"
                          >
                            {tech}
                          </span>
                        ))}
                    ...
                    <div className="flex gap-3 mt-4">
                      {project.githubUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-blue-300 hover:text-sky-400 flex items-center gap-1"
                        >
                      ...
                      {project.liveUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-blue-300 hover:text-cyan-300 flex items-center gap-1"
                        >
              ...
              {user.experiences.map((exp: any) => (
                <FadeInView key={exp.id}>
                  <div className="relative">
                    ...
                    {exp.description && (
                      <p className="text-blue-200/80 mt-3 text-sm leading-relaxed">{exp.description}</p>
                    )}
                  </div>
                </FadeInView>
              ))}
      ...
      <footer className="py-8 text-center text-blue-400/60 text-sm border-t border-blue-500/10">
