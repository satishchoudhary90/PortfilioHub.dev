    ...
    <div className="min-h-screen bg-black text-green-400 font-mono">
      ...
      <section className="min-h-screen flex items-center justify-center relative px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl w-full mx-auto relative z-10">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-xl border border-green-500/30 bg-black/80 backdrop-blur p-6 sm:p-8">
            ...
            <div className="space-y-2 text-xs sm:text-sm">
              ...
              <p className="text-green-300 text-xl sm:text-2xl md:text-3xl font-bold break-words"><TypeWriter text={user.name || "Developer"} /></p>
              <p><span className="text-green-600">$</span> cat role.txt</p>
              <p className="text-green-300 text-base sm:text-lg md:text-xl break-words">{user.headline || "Full Stack Developer"}</p>
      ...
      {user.bio && (
        <section id="about" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView>
            <div className="rounded-xl border border-green-500/20 bg-black/60 p-4 sm:p-6">
              <p className="text-green-600 text-xs mb-3">{"/* about.md */"}</p>
              <h2 className="text-xl sm:text-2xl font-bold text-green-300 mb-4"># About</h2>
              <p className="text-green-400/80 leading-relaxed text-sm sm:text-base">{user.bio}</p>
            </div>
          </FadeInView>
        </section>
      )}
      ...
      {user.skills?.length > 0 && (
        <section id="skills" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView>
            <h2 className="text-xl sm:text-2xl font-bold text-green-300 mb-8"># Skills</h2>
          </FadeInView>
          <StaggerContainer className="flex flex-wrap gap-2 sm:gap-3">
            {user.skills.map((skill: any) => (
              <StaggerItem key={skill.id}>
                <motion.div whileHover={{ scale: 1.05 }} className="p-2 sm:p-3 rounded-lg border border-green-500/20 bg-green-500/5 min-w-0">
      ...
      {user.projects?.length > 0 && (
        <section id="projects" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView>
            <h2 className="text-xl sm:text-2xl font-bold text-green-300 mb-8"># Projects</h2>
          </FadeInView>
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {user.projects.map((project: any) => (
              <StaggerItem key={project.id}>
                <motion.div whileHover={{ scale: 1.02, borderColor: "rgba(34, 197, 94, 0.5)" }} transition={{ duration: 0.3 }} className="rounded-xl border border-green-500/20 bg-black/60 p-4 sm:p-5 space-y-3 overflow-hidden transition-all duration-300">
                  <div className="flex items-center gap-2 text-xs text-green-600">
                    <Terminal className="h-3 w-3" />
                    <span>project/{project.title.toLowerCase().replace(/\s+/g, "-")}</span>
                  </div>
                  <h3 className="font-bold text-green-300 text-base sm:text-lg">{project.title}</h3>
                  <p className="text-xs sm:text-sm text-green-400/60 line-clamp-2">{project.description}</p>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {project.techStack?.map((tech: string) => (
                      <span key={tech} className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] rounded border border-green-500/30 text-green-400">{tech}</span>
                    ))}
                  </div>
      ...
      {user.experiences?.length > 0 && (
        <section id="experience" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView><h2 className="text-xl sm:text-2xl font-bold text-green-300 mb-8"># Experience</h2></FadeInView>
          <div className="space-y-6">
            {user.experiences.map((exp: any, i: number) => (
              <FadeInView key={exp.id} delay={i * 0.1}>
                <div className="p-4 sm:p-5 rounded-xl border border-green-500/20 bg-black/60 overflow-hidden">
                  <p className="text-xs text-green-600 mb-2 break-all">{`// ${formatDate(exp.startDate)} - ${exp.current ? "present" : exp.endDate ? formatDate(exp.endDate) : ""}`}</p>
                  <h3 className="font-bold text-green-300 text-base sm:text-lg">{exp.position}</h3>
                  <p className="text-green-500 text-xs sm:text-sm">@{exp.company}</p>
                  {exp.description && <p className="text-xs sm:text-sm text-green-400/60 mt-3 break-words">{exp.description}</p>}
                </div>
              </FadeInView>
            ))}
          </div>
      ...
      <section id="contact" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
        <FadeInView>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-green-300 mb-8 text-center"># Contact</h2>
          {sent ? (
