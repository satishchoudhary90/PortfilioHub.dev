  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white overflow-hidden">
      ...
      <section className="min-h-screen flex items-center relative px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-8 sm:gap-12 items-center relative z-10">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-base sm:text-lg md:text-xl text-gray-400 mt-4">{user.headline || "Building the future with code"}</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="hidden md:block">
            <div className="relative">
              ...
              {user.image ? (
                <img src={user.image} alt={user.name} className="relative rounded-3xl w-full max-w-full aspect-square object-cover border border-white/10" />
              ) : (
      ...
      {/* About */}
      {user.bio && (
        <section id="about" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full relative scroll-mt-20">
          <FadeInView>
      ...
      {user.skills?.length > 0 && (
        <section id="skills" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView><h2 className="text-2xl sm:text-3xl font-bold mb-12 text-center">Tech Stack</h2></FadeInView>
          <StaggerContainer className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {user.skills.map((skill: any) => (
              <StaggerItem key={skill.id}>
                <motion.div whileHover={{ scale: 1.05, y: -2 }} className="px-3 py-2 sm:px-5 sm:py-3 rounded-xl bg-gradient-to-r from-white/5 to-white/10 border border-white/10 backdrop-blur text-xs sm:text-sm font-medium text-white transition-transform duration-300">
                  {skill.name}
                </motion.div>
              </StaggerItem>
            ))}
      ...
      {user.projects?.length > 0 && (
        <section id="projects" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView><h2 className="text-2xl sm:text-3xl font-bold mb-8 sm:mb-12 text-center">Featured Work</h2></FadeInView>
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {user.projects.map((project: any) => (
              <StaggerItem key={project.id}>
                <motion.div whileHover={{ y: -8, scale: 1.02 }} transition={{ duration: 0.3 }} className="group rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl overflow-hidden">
                  {project.imageUrl && (
                    <div className="aspect-video overflow-hidden">
                      <img src={project.imageUrl} alt={project.title} className="w-full h-full max-w-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    </div>
                  )}
                  <div className="p-4 sm:p-6 space-y-4">
                    ...
                    <p className="text-xs sm:text-sm text-gray-400 line-clamp-2">{project.description}</p>
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {project.techStack?.map((tech: string) => (
                        <span key={tech} className="px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs rounded-lg bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-white/10 text-gray-300">{tech}</span>
                      ))}
                    </div>
                  </div>
                </motion.div>
      ...
      {user.experiences?.length > 0 && (
        <section id="experience" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
          <FadeInView><h2 className="text-2xl sm:text-3xl font-bold mb-8 sm:mb-12 text-center">Career Journey</h2></FadeInView>
          <div className="space-y-6">
            {user.experiences.map((exp: any, i: number) => (
              <FadeInView key={exp.id} delay={i * 0.1}>
                <div className="p-4 sm:p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl overflow-hidden">
      ...
      {/* Contact */}
      <section id="contact" className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full relative scroll-mt-20">
        <FadeInView>
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-8 sm:mb-12">
              <h2 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Let&apos;s Connect</h2>
              <p className="text-gray-400 mt-3 text-sm sm:text-base">Got a project in mind? Let&apos;s build something amazing.</p>
            </div>
            {sent ? (
              <div className="text-center py-8"><p className="text-lg sm:text-xl text-cyan-400">Message sent successfully!</p></div>
            ) : (
