...
export default function TechnicalResume({ data, editable, onEdit }: ResumeProps) {
  if (!data) return null;
  ...
  return (
    <div className="bg-gray-950 text-gray-200 p-10 space-y-6 font-mono w-[794px] min-h-[1123px] mx-auto shadow-2xl">
        ...
        <div className="flex items-start gap-4">
          ...
          {(data.headline || editable) && (
            <>
              ...
              <EditableText value={data.headline} field="headline" editable={editable} onEdit={onEdit} className="text-green-400 block" tag="p" />
            </>
          )}
          <p><span className="text-green-500">$</span> cat contact.json</p>
      ...
      {(data.bio || editable) && (
        <div>
          ...
          <EditableText value={data.bio} field="bio" editable={editable} onEdit={onEdit} className="text-xs text-gray-400 leading-relaxed pl-4 border-l-2 border-green-900 block" tag="p" />
        </div>
      )}
      ...
      {data.skills?.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {Object.entries(groupedSkills).map(([cat, skills]: [string, any]) => (
              <div key={cat}>
                <p className="text-xs text-green-600 uppercase mb-1.5">{`// ${cat}`}</p>
                <div className="flex flex-wrap gap-1.5">
                  {skills.map((s: any) => (
                    <span key={s.id} className="px-2 py-0.5 text-[11px] rounded border border-green-500/30 text-green-300 bg-green-500/5">
                      {s.name}
                    </span>
                  ))}
                </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {data.experiences.map((exp: any) => (
              <div key={exp.id} className="border border-gray-800 rounded-lg p-4 bg-gray-900/50">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[10px] text-gray-600 font-mono">
                    {formatDate(exp.startDate)} → {exp.current ? "now" : exp.endDate ? formatDate(exp.endDate) : ""}
                  </span>
                </div>
                {exp.description && (
        ...
        <div>
          ...
          <div className="space-y-3">
            {data.projects.map((proj: any) => (
              <div key={proj.id} className="border border-gray-800 rounded-lg p-3 bg-gray-900/50">
                <div className="flex justify-between items-start">
                  <h3 className="text-sm font-bold text-white">{proj.title}</h3>
                  <div className="flex gap-2 text-[10px]">
                    {proj.githubUrl && <a href={proj.githubUrl} className="text-cyan-400 hover:underline">repo</a>}
                    {proj.liveUrl && <a href={proj.liveUrl} className="text-green-400 hover:underline">live</a>}
                  </div>
                </div>
                {proj.description && <p className="text-xs text-gray-400 mt-1">{proj.description}</p>}
                {proj.techStack?.length > 0 && (
                  <p className="text-[10px] text-gray-600 mt-1">{proj.techStack.join(" | ")}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {data.educations.map((edu: any) => (
              <div key={edu.id} className="border border-gray-800 rounded-lg p-3 bg-gray-900/50">
                ...
                <p className="text-[10px] text-gray-600 mt-1">
                  {formatDate(edu.startDate)} → {edu.current ? "now" : edu.endDate ? formatDate(edu.endDate) : ""}
          ...
          <div className="flex flex-wrap gap-3 text-xs">
            {data.socialLinks.map((link: any) => (
              <span key={link.id} className="text-cyan-400">
                [{link.platform}](<span className="text-gray-500">{link.url}</span>)
              </span>
            ))}
          </div>
        </div>
