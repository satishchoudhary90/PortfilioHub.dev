...
function SkillCircle({ name, level = 85, color = "#059669" }: { name: string; level?: number; color?: string }) {
  const r = 36;
...
export default function InfographicResume({ data, editable, onEdit }: ResumeProps) {
  if (!data) return null;
  ...
  const yearsExp = data.experiences?.length
    ? Math.max(
        ...
        ...data.experiences.map((e: any) => {
          ...
          return Math.floor((end.getTime() - start.getTime()) / (365 * 24 * 60 * 60 * 1000));
        })
      )
    : 0;
  ...
  return (
    <div className="w-[794px] min-h-[1123px] mx-auto shadow-2xl bg-white p-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="text-center p-4 rounded-xl" style={{ backgroundColor: `${accents.emerald}15` }}>
          <div className="text-3xl font-bold" style={{ color: accents.emerald }}>
            {projectCount}
          </div>
          <div className="text-xs text-gray-600 mt-0.5">Projects</div>
        </div>
        <div className="text-center p-4 rounded-xl" style={{ backgroundColor: `${accents.amber}15` }}>
          <div className="text-3xl font-bold" style={{ color: accents.amber }}>
            {yearsExp}+
          </div>
          <div className="text-xs text-gray-600 mt-0.5">Years Exp</div>
        </div>
        <div className="text-center p-4 rounded-xl" style={{ backgroundColor: `${accents.blue}15` }}>
      ...
      {(data.bio || editable) && (
        <section className="mb-8">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-2" style={{ color: accents.emerald }}>
            About
          </h2>
      ...
      {data.skills?.length > 0 && (
        <section className="mb-8">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: accents.blue }}>
            Skills
          </h2>
      ...
      {data.experiences?.length > 0 && (
        <section className="mb-8">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: accents.emerald }}>
            Experience
          </h2>
            ...
            <div className="space-y-6 pl-8">
              {data.experiences.map((exp: any) => (
                <div key={exp.id} className="relative">
                  ...
                  />
                  <div className="bg-gray-50 rounded-lg p-4 border-l-4" style={{ borderLeftColor: "#059669" }}>
      ...
      {data.projects?.length > 0 && (
        <section>
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: accents.violet }}>
            Projects
          </h2>
          <div className="space-y-3">
            {data.projects.map((proj: any) => (
              <div key={proj.id} className="bg-violet-50/50 rounded-lg p-4 border-l-4 border-violet-500">
                <div className="flex justify-between items-baseline">
                  <h3 className="font-semibold text-gray-900">{proj.title}</h3>
                  <div className="flex gap-2 text-xs" style={{ color: accents.violet }}>
                ...
                {proj.techStack?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {proj.techStack.map((tech: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-violet-100 text-violet-700">{tech}</span>
                    ))}
                  </div>
                )}
              </div>
      ...
      {data.educations?.length > 0 && (
        <section>
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: accents.amber }}>
            Education
          </h2>
