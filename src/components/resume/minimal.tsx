...
export default function MinimalResume({ data, editable, onEdit }: ResumeProps) {
  ...
  return (
    <div className="bg-white text-gray-900 p-4 sm:p-6 lg:p-10 space-y-3 sm:space-y-4 lg:space-y-5 font-sans w-full max-w-[794px] min-h-screen print:min-h-[1123px] mx-auto shadow-sm lg:shadow-2xl">
      ...
      {(data.bio || editable) && (
        <EditableText
          ...
          editable={editable}
          onEdit={onEdit}
          className="text-sm text-gray-600 leading-relaxed text-center max-w-xl mx-auto block"
          tag="p"
        />
      )}
            ...
            {data.projects.map((proj: any) => (
              <div key={proj.id} className="flex justify-between items-start">
                ...
                <div className="flex gap-2 text-[11px] text-gray-400 whitespace-nowrap mt-0.5">
                  {proj.githubUrl && (
                    <a href={proj.githubUrl} className="hover:text-gray-600">
                      GitHub
                    </a>
                  )}
                  {proj.liveUrl && (
                    <a href={proj.liveUrl} className="hover:text-gray-600">
                      Live
                    </a>
                  )}
                </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.skills.map((skill: any) => (
              <span
                key={skill.id}
                className="px-2 py-0.5 text-xs text-gray-600 bg-gray-50 rounded border border-gray-100"
              >
                {skill.name}
              </span>
            ))}
