...
export default function ModernSplitResume({ data, editable, onEdit }: ResumeProps) {
  if (!data) return null;

  const TEAL = "#0d9488";

  return (
    <div className="w-[794px] min-h-[1123px] mx-auto shadow-2xl flex overflow-hidden rounded-lg">
      {/* Left column — 35% teal */}
      <div
      ...
      >
        ...
        {data.skills?.length > 0 && (
          <div className="mt-8">
            <h2 className="text-xs font-bold uppercase tracking-wider text-white/80 mb-3">Skills</h2>
        ...
        {data.languages && data.languages.length > 0 && (
          <div className="mt-6">
            <h2 className="text-xs font-bold uppercase tracking-wider text-white/80 mb-2">Languages</h2>
            <div className="space-y-1 text-sm">
              {data.languages.map((lang: any) => (
                <div key={lang.id}>{typeof lang === "string" ? lang : lang.name}</div>
              ))}
            </div>
          </div>
        )}
        ...
        {data.socialLinks?.length > 0 && (
          <div className="mt-6">
            <h2 className="text-xs font-bold uppercase tracking-wider text-white/80 mb-2">Links</h2>
            <div className="space-y-1 text-sm">
      ...
      <div className="flex-1 bg-white p-8 text-gray-900">
        {(data.bio || editable) && (
          <section className="mb-8">
            <h2 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: TEAL }}>
              Summary
            </h2>
        ...
        {data.experiences?.length > 0 && (
          <section className="mb-8">
            <h2 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: TEAL }}>
              Experience
            </h2>
          ...
          <section>
            ...
            <div className="space-y-5">
              {data.projects.map((proj: any) => (
                <div key={proj.id} className="rounded-lg border border-gray-200 p-4">
                  <div className="flex justify-between items-baseline">
