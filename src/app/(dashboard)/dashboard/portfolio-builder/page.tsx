...
const platformIcons: Record<string, any> = { github: Github, linkedin: Linkedin, twitter: Twitter, website: Globe, email: Mail };
const skillCategories = ["frontend", "backend", "database", "devops", "mobile", "design", "general"];
...
type SectionKey = "projects" | "skills" | "experience" | "education" | "socialLinks";

const sectionConfig: { key: SectionKey; label: string; icon: any; color: string }[] = [
  { key: "projects", label: "Projects", icon: FolderKanban, color: "text-blue-400" },
  ...
  { key: "education", label: "Education", icon: GraduationCap, color: "text-purple-400" },
  { key: "socialLinks", label: "Social Links", icon: Share2, color: "text-pink-400" },
];
...
export default function PortfolioBuilderPage() {
  ...
  const [collapsed, setCollapsed] = useState<Record<SectionKey, boolean>>({ projects: false, skills: false, experience: false, education: false, socialLinks: false });
  ...
  function startAdd(section: SectionKey) {
    const defaults: Record<SectionKey, any> = {
      projects: { title: "", description: "", techStack: [], githubUrl: "", liveUrl: "", imageUrl: "", featured: false, _techInput: "" },
      skills: { name: "", level: 70, category: "frontend" },
      experience: { company: "", position: "", description: "", location: "", startDate: "", endDate: "", current: false },
      education: { institution: "", degree: "", field: "", description: "", startDate: "", endDate: "", current: false, grade: "" },
  ...
  function apiEndpoint(section: SectionKey): string {
    const map: Record<SectionKey, string> = { projects: "/api/projects", skills: "/api/skills", experience: "/api/experience", education: "/api/education", socialLinks: "/api/social-links" };
    return map[section];
  }
  ...
  function handleDragEnd(section: SectionKey, event: DragEndEvent) {
    ...
    const setter = { projects: setProjects, skills: setSkills, experience: setExperiences, education: setEducations, socialLinks: setSocialLinks }[section];
    const items = { projects, skills, experience: experiences, education: educations, socialLinks }[section];
  ...
  function addTechTag() {
    ...
    if (tag && !formData.techStack.includes(tag)) {
      setFormData((p: any) => ({ ...p, techStack: [...p.techStack, tag], _techInput: "" }));
    }
  }
    ...
    switch (section) {
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                <Input value={formData._techInput || ""} onChange={e => setFormData((p: any) => ({ ...p, _techInput: e.target.value }))} onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addTechTag())} placeholder="Add tech..." className="flex-1" />
                <Button type="button" size="sm" variant="secondary" onClick={addTechTag}>Add</Button>
              </div>
              {formData.techStack?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {formData.techStack.map((t: string) => (
                    <Badge key={t} variant="secondary" className="gap-1 cursor-pointer" onClick={() => removeTechTag(t)}>{t} <X className="h-3 w-3" /></Badge>
                  ))}
                </div>
              )}
            </div>
            <div className="flex items-center gap-2"><Switch checked={formData.featured || false} onCheckedChange={v => setFormData((p: any) => ({ ...p, featured: v }))} /><Label className="text-xs">Featured project</Label></div>
          </div>
        );
      case "skills":
        return (
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label className="text-xs">Level: {formData.level || 70}%</Label>
                <input type="range" min={0} max={100} value={formData.level || 70} onChange={e => setFormData((p: any) => ({ ...p, level: Number(e.target.value) }))} className="w-full mt-2 accent-indigo-500" />
              </div>
            </div>
          </div>
        );
      case "experience":
        return (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">Position *</Label><Input value={formData.position || ""} onChange={e => setFormData((p: any) => ({ ...p, position: e.target.value }))} placeholder="Software Engineer" /></div>
              <div><Label className="text-xs">Company *</Label><Input value={formData.company || ""} onChange={e => setFormData((p: any) => ({ ...p, company: e.target.value }))} placeholder="Company name" /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div><Label className="text-xs">Start Date *</Label><Input type="date" value={formData.startDate || ""} onChange={e => setFormData((p: any) => ({ ...p, startDate: e.target.value }))} /></div>
              <div><Label className="text-xs">End Date</Label><Input type="date" value={formData.endDate || ""} onChange={e => setFormData((p: any) => ({ ...p, endDate: e.target.value }))} disabled={formData.current} /></div>
      ...
      case "education":
        return (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">Degree *</Label><Input value={formData.degree || ""} onChange={e => setFormData((p: any) => ({ ...p, degree: e.target.value }))} placeholder="B.Tech, M.Sc..." /></div>
              <div><Label className="text-xs">Institution *</Label><Input value={formData.institution || ""} onChange={e => setFormData((p: any) => ({ ...p, institution: e.target.value }))} placeholder="University name" /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div><Label className="text-xs">Field</Label><Input value={formData.field || ""} onChange={e => setFormData((p: any) => ({ ...p, field: e.target.value }))} placeholder="Computer Science" /></div>
              <div><Label className="text-xs">Grade</Label><Input value={formData.grade || ""} onChange={e => setFormData((p: any) => ({ ...p, grade: e.target.value }))} placeholder="3.8/4.0" /></div>
              <div className="flex items-end pb-2 gap-2"><Switch checked={formData.current || false} onCheckedChange={v => setFormData((p: any) => ({ ...p, current: v, endDate: v ? "" : p.endDate }))} /><Label className="text-xs">Current</Label></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">Start Date *</Label><Input type="date" value={formData.startDate || ""} onChange={e => setFormData((p: any) => ({ ...p, startDate: e.target.value }))} /></div>
        ...
        return (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label className="text-xs">URL *</Label><Input value={formData.url || ""} onChange={e => setFormData((p: any) => ({ ...p, url: e.target.value }))} placeholder="https://..." /></div>
            </div>
          </div>
        );
    }
      ...
      {sectionConfig.map((section, idx) => {
        ...
        return (
          <FadeIn key={section.key} delay={idx * 0.05}>
            <Card>
              <CardHeader className="pb-3 cursor-pointer" onClick={() => toggleSection(section.key)}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={(e) => { e.stopPropagation(); startAdd(section.key); if (isCollapsed) toggleSection(section.key); }}
                    >
                ...
                {!isCollapsed && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}>
                    <CardContent className="pt-0 space-y-2">
                      {isEditing && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/20 space-y-3">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-indigo-400">{editingItem.id ? "Edit" : "Add"} {section.label.replace(/s$/, "")}</p>
                            <div className="flex gap-2">
                              <Button size="sm" variant="secondary" onClick={cancelEdit}><X className="h-3.5 w-3.5 mr-1" /> Cancel</Button>
                              <Button size="sm" onClick={saveItem}><Check className="h-3.5 w-3.5 mr-1" /> Save</Button>
                            </div>
                          </div>
